...
export default function FeedCard({ post }: { post: Post }) {
  ...
  return (
    <article className="group relative flex flex-col bg-[#0f1117] border border-white/[0.06] rounded-2xl overflow-hidden shadow-xl shadow-black/40 hover:border-white/[0.12] hover:shadow-2xl hover:shadow-black/60 transition-all duration-300">
      ...
      {post.prompt_image || post.raw_image ? (
        <div className="relative w-full aspect-[16/9] overflow-hidden bg-[#0a0c10]">
          ...
          {post.ai_model && (
            <span
              className={`absolute bottom-3 left-3 text-[10px] font-semibold px-2 py-0.5 rounded-full border backdrop-blur-md ${modelColor(post.ai_model)}`}
            >
              {post.ai_model}
            </span>
          )}
        ...
        <div className="relative w-full aspect-[16/9] bg-[#0a0c10] overflow-hidden flex items-center justify-center">
          <div
            ...
            style={{
              backgroundImage:
                "radial-gradient(circle at 30% 40%, #1e3a5f 0%, transparent 55%), radial-gradient(circle at 75% 70%, #1a1040 0%, transparent 55%)",
            }}
          />
          <svg
